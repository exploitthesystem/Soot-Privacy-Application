...
public class SimpleAPILookupTransformer extends BodyTransformer{
	...
	protected void internalTransform(Body body, String string, Map map) {
		...
		while(iter.hasNext()){
			...
			if (s instanceof InvokeStmt && !s.getInvokeExpr().getMethod().getDeclaringClass().isApplicationClass()){
				//System.out.println("****This is an API: " + s.getInvokeExpr().getMethod());
				apis.add(s.getInvokeExpr().getMethod().toString());
			}
